...
public class BookService {
  ...
  public Book createBook(String title, String author, String subject, String extension,
                         byte[] content, double price) {
    String uniqueFilename = UUID.randomUUID().toString();
    String filePath = String.format("%s/%s.%s", outFolder, uniqueFilename, extension);
    Book book = new Book(null, title, subject, author, filePath, uniqueFilename, null, price, true);
  ...
  public List<String> getPreviewImages(Book book) {
    ...
    for (int i = 0; i < 4; i++) {
      String base64Image = Base64.getEncoder()
          .encodeToString(bookCoverService.generateCover(fileContent, i));
  ...
  public Page<Book> getAllBooks(Pageable pageable) {
    return bookRepository.findAllByIsAvailableIsTrue(pageable);
  }

  public Page<Book> searchBooks(String searchTerm, Pageable pageable) {
    return bookRepository.findByTitleContainingIgnoreCaseOrGenreContainingIgnoreCaseOrAuthorContainingIgnoreCase(
        searchTerm,
        searchTerm,
        searchTerm,
        pageable);
